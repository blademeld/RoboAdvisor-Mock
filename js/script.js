...
$(function() {
	...
	makeAmChart('chart2', 'risk');
	makeAmChart('chart3', 'succ', 3);
	makeHighchart('chart4', 0);					//Hichart는 데이타 한 세트만 출력
	makeHighchart('chart5', 4);
	makeGaugeChart('chart6', 0);	
...
//Hichart 함수
function makeHighchart(chartName, arrayIndex){
	$('#'+chartName).highcharts({
	  chart: {
		zoomType: 'xy'
	  },
	  title: {
		text: jsonData[arrayIndex]["name"]
	  },
	  credits: {
		enabled: false
	  },
	  colors: ['rgb(178,222,247)', 'rgb(255,223,124)'],
	  xAxis: [{
		categories: [''],
		crosshair: true
	  }],
	  yAxis: [{ // Primary yAxis
		...
		labels: {
		  formatter: function() {
			return Highcharts.numberFormat(this.value, 0, '.', ','); //returns the y axis label
		  },
		  style: {
			color: Highcharts.getOptions().colors[0]
		  }
	  ...
	  series: [{
		...
		type: 'column',
		yAxis: 1,
		data: [jsonData[arrayIndex]["profit"]],
		tooltip: {
		  valueSuffix: ' 원'
		}
...
function makeGaugeChart(chartName, arrayIndex){	
	if (jsonData[arrayIndex]['succ'] < 35){
		var note = '낮음';
	} else if (jsonData[arrayIndex]['succ'] < 65){
		var note = '보통';
	} else {
		var note = '높음';
	}

    var gauge = new Gauge(document.getElementById(chartName)).setOptions({angle:0});
    gauge.maxValue = 100; // set max gauge value
    // gauge.animationSpeed = 32; // set animation speed (32 is default value)
    gauge.set(jsonData[arrayIndex]['succ']); // set actual value
    gauge.setTextField(document.getElementById("gauge-textfield"));
    document.getElementById("chart6_title").innerHTML = jsonData[arrayIndex]['name'];
    document.getElementById("chart6_note").innerHTML = note;
    // document.getElementById("gauge-textfield-unit").innerHTML = "%";
}
